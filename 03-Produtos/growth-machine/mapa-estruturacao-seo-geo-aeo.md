...
seguem a sequência do catálogo mestre (**RN-123+**), para não colidir com a
...
|---|---|---|---|---|---|
| Extrabilidade / answer-first (bloco autocontido) | — | Dim 2 (2C) | 🟡 | Criar **RN-123** (extrabilidade) + subcheck em 2C | Fase 2 |
| Resposta única/inequívoca (AEO) | — | Dim 2 (2C) | 🔴 | Nova sub-dimensão **2D–AEO** + RN-124 | Fase 2 |
| Meta description ≤160 / title ≤60 | — | Dim 2 | 🟡 | Checagem **determinística** em Dim 2 + RN-125 | MVP (barato) |
| Priorização de AEO por nicho | — (usa briefing) | Dim 2/2D | 🔴 | Regra de priorização por nicho (reusa briefing) + RN-126 | MVP (barato) |

## Princípio 2 — Anti-canibalização como regra estrutural
| Oportunidade | RN atual | Dim atual | Estado | Ação | Fase |
|---|---|---|---|---|---|
| **Anti-doorway / scaled content** (core 2026) | RN-14, RN-15, RN-85 (só no estudo) | Dim 1 + Dim 3 | 🟡 | Nova **RN-127**: limite de variação por intenção + camada editorial (bate no C3 Gregory) | MVP (risco alto) |
| Canibalização **www × loja** (subdomínios) | RN-15/85 (só estudo) | Dim 3 | 🟡→🔴 | Estender **Dim 3** para arquitetura multi-subdomínio + RN-128 | MVP |
...
| Oportunidade | RN atual | Dim atual | Estado | Ação | Fase |
|---|---|---|---|---|---|
| Sinais **E-E-A-T on-page** (autor/sobre/fontes) | — | Dim 2 | 🔴 | Nova subcheck **2E** (E-E-A-T) + RN-129 | MVP (core 2026) |
| Conteúdo **original / information gain** | RN-14 (padrão SERP = risco) | Dim 2 | 🟡 | Sinal de ineditismo em Dim 2 + RN-130 ([[05-Backlog/gm-sinal-conteudo-original]]) | Fase 2 |
| Autoridade / **ofensiva de backlink** | — (Dim 8 só disavow) | Dim 8 | 🟡 | Estender **Dim 8** reativa→propositiva + RN-131 | Fase 2 |
...
| Oportunidade | RN atual | Dim atual | Estado | Ação | Fase |
|---|---|---|---|---|---|
| **Medição de citação em LLM / SoV** (AI Overview) | — | 2C (isolada no Ideal Tracker) | 🔴 | **Nova Dimensão 11 — GEO/Citação** alimentada pelo Ideal Tracker + RN-132 ([[05-Backlog/gm-integrar-sinal-citacao-llm-ideal-tracker]]) | Fase 2 |
...
| Tráfego de **origem IA** (GA4 segmentado) | — | Motor / Dim 8 | 🟡 | Segmentar origem IA no Motor de Percepção + RN-133 ([[05-Backlog/gm-segmentar-trafego-origem-ia]]) | Fase 2 |
| Crawl **log real** (indexação de fato) | — | Dim 7/9 | 🔴 | Nova subcheck de log de servidor em Dim 7 + RN-134 | Fase 2 |
...
| Oportunidade | RN atual | Dim atual | Estado | Ação | Fase |
|---|---|---|---|---|---|
| **Controle de crawler de IA** (GPTBot/ClaudeBot/Perplexity/Google-Extended) | RN-82 (só llms.txt) | Dim 7 + Sentinela | 🟡 | **Incorporar o PRD robots do Gregory** à Dim 7/Sentinela + RN-135 | MVP (barato, já existe no Gregory) |
...
| Autoridade de **entidade** (sameAs/Knowledge Graph) | RN-117 (schema base) | Dim 6 + Dim 8 | 🟡 | `sameAs` no schema (Dim 6) + presença de entidade (Dim 8) + RN-136 ([[05-Backlog/gm-checagem-presenca-entidade]]) | Fase 2 |
| Presença **off-site** (Reddit/fóruns/reviews) | — | — | 🔴 | Capability nova (Dim 8 estendida ou Dim 12) | Fase 2/3 |
| **Pilares agênticos** (accessibility tree, CLS, WebMCP) | RN-82 (1 de 6) | Dim 5 + Dim 7 | 🔴 | CLS/accessibility em Dim 5; WebMCP nova checagem + RN-137 ([[05-Backlog/gm-implementar-pilares-agenticos-webmcp]]) | Fase 2 |
| Conteúdo importante **preso em PDF** | — | Dim 2/7 | 🔴 | Sinal "migrar ativo de PDF→HTML" + RN-138 | MVP (barato) |
...
RN-84) · ~16 RNs novas propostas (RN-123 a RN-138) · 2 sub-dimensões novas
...
- **MVP / agora:** ajuste RN-84 (poda), RN-07 (CWV real), anti-doorway
  (RN-127) + camada editorial, controle de crawler de IA (RN-135, barato),
  meta/title (RN-125), priorização AEO por nicho (RN-126), PDF→HTML (RN-138).
- **Fase 2:** Dim 11 (medição GEO), extrabilidade/AEO (RN-123/124),
  entidade (RN-136), pilares agênticos (RN-137), ofensiva de backlink
  (RN-131), crawl-log (RN-134), tráfego IA (RN-133), off-site.

## Próximo passo
Com este mapa, dá para (a) abrir os itens **NOVO** (RN-127/128/129/135/138)
