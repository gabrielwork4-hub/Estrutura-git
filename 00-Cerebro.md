...
- [[03-Produtos/growth-machine/prd-v2-mvp]] (2026-07-08, **consolidado em PRDFINAL 2026-07-10**) — **histórico**: os 13 blocos preenchidos, RF numerados (RF-00 a RF-46), User Stories/CA dos fluxos críticos, RNs reconciliadas (supersedidas, threshold Dim 1, Bright Data) e escopo SEO/GEO/AEO no nível de MVP (GEO = checklist; medição = Fase 2, F-28). Anti-canibalização por design: RNs seguem morando só no catálogo; o PRD referencia. Deixou de ser a cópia de leitura para build — ver bullet abaixo.
- [[PRDFINAL]] (2026-07-10) — **PRD final (v1.0)**: absorve o `prd-v2-mvp` acima com todas as reconciliações já como texto principal (pesos 40/40/20 sob revisão, Cluster Wrapping, Portão de Diferenciação Real) + **8 RFs novas (RF-47 a RF-54)** formalizando os itens `RN-SGA-*`/`RN-EST-*` já marcados "✅ MVP" no checklist, que antes só existiam em prosa. Traz seção dedicada de **aderência a boas práticas Google 2026** (Helpful Content, E-E-A-T, Scaled Content Abuse/Doorway) cruzada com a estrutura de RN/Dimensão — veredito: os 3 sistemas nomeados estão **desenhados**, nenhum **implementado** ainda; o mais urgente (doorway) tem antídoto no PRD mas falta a auditoria retroativa da carteira (Frente Z1) para medir a exposição real. Não declara o produto "pronto para codar" — seção final lista o que segue aberto (7 questões de integração, golden-set, ERD).
...
- [[03-Produtos/mpi-plus]] — pré-requisito de carteira da Growth Machine (RN-108); refatoração de processo e palavras-chave em 3 histórias: geração de termos (**enviada ao Jira em 2026-07-03**, [[03-Produtos/mpi-plus/historia-prompt-inversao-fluxo-keywords]], resolve o gargalo do fluxo), endurecimento anti-concatenação/GEO genérico (oficial, motivada por QA de produção real, [[03-Produtos/mpi-plus/historia-prompt-endurecimento-anti-concatenacao-geo]]) e arquitetura de site (oficial, aguardando envio, [[03-Produtos/mpi-plus/historia-jira-prompt-estudo-keywords-v3]]); sem PRD próprio encontrado no Drive ainda. **UI/UX do painel mapeada em 2026-07-13** ([[03-Produtos/mpi-plus/mapa-funcionalidades-painel]]): Dashboard/Clientes (Hub do Cliente: Resumo/Briefing/Estudo/Relatório)/Relatórios/Usuários/Configurações — confirma que o relatório mensal (RN-105/RN-106 do GM) e a aprovação do cliente (RN-103) nascem nessas telas concretas. **Posicionamento confirmado pelo usuário:** o MPI Plus é o **sistema interno do Grupo**, centraliza o cliente do onboarding até a validação final (o GM roda por cima como camada de diagnóstico, não é o dono do histórico do cliente); geração do relatório mensal é **automática**. Segue em aberto: papéis internos do MPI+ sem de-para com as personas do GM, módulos do portal do cliente, logs.
